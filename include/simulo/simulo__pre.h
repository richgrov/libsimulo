...
__attribute__((__import_name__("simulo_set_object_ptrs"))) extern void
simulo_set_object_ptrs(uint32_t id, void *self);

__attribute__((__import_name__("simulo_add_object_child"))) extern void
simulo_add_object_child(uint32_t parent, uint32_t child);

__attribute__((__import_name__("simulo_num_children"))) extern uint32_t
simulo_num_children(uint32_t id);
...
__attribute__((__import_name__("simulo_drop_object"))) extern void
simulo_drop_object(uint32_t);

__attribute__((
    __import_name__("simulo_create_rendered_object"))) extern uint32_t
simulo_create_rendered_object(uint32_t material);

__attribute__((
    __import_name__("simulo_set_rendered_object_material"))) extern void
simulo_set_rendered_object_material(uint32_t id, uint32_t material);

__attribute__((
    __import_name__("simulo_set_rendered_object_transform"))) extern void
simulo_set_rendered_object_transform(uint32_t id, const float *transform);
...
public:
  ...
  glm::vec2 left_eye() const { return glm::vec2(data_[2], data_[3]); }

  glm::vec2 right_eye() const { return glm::vec2(data_[4], data_[5]); }

  glm::vec2 left_ear() const { return glm::vec2(data_[6], data_[7]); }

  glm::vec2 right_ear() const { return glm::vec2(data_[8], data_[9]); }

  glm::vec2 left_shoulder() const { return glm::vec2(data_[10], data_[11]); }

  glm::vec2 right_shoulder() const { return glm::vec2(data_[12], data_[13]); }

  glm::vec2 left_elbow() const { return glm::vec2(data_[14], data_[15]); }

  glm::vec2 right_elbow() const { return glm::vec2(data_[16], data_[17]); }

  glm::vec2 left_wrist() const { return glm::vec2(data_[18], data_[19]); }

  glm::vec2 right_wrist() const { return glm::vec2(data_[20], data_[21]); }

  glm::vec2 left_hip() const { return glm::vec2(data_[22], data_[23]); }

  glm::vec2 right_hip() const { return glm::vec2(data_[24], data_[25]); }

  glm::vec2 left_knee() const { return glm::vec2(data_[26], data_[27]); }

  glm::vec2 right_knee() const { return glm::vec2(data_[28], data_[29]); }

  glm::vec2 left_ankle() const { return glm::vec2(data_[30], data_[31]); }

  glm::vec2 right_ankle() const { return glm::vec2(data_[32], data_[33]); }

private:
  friend void ::simulo__pose(int id, bool alive);

  Pose(float *data) { std::memcpy(data_.data(), data, sizeof(data_)); }
...
class Material {
public:
  Material(uint32_t image, float r, float g, float b)
      : simulo__id(simulo_create_material(image, r, g, b)) {}
...
public:
  ...
  virtual ~Object() { simulo_drop_object(simulo__id); }
  ...
  void delete_from_parent() { simulo_remove_object_from_parent(simulo__id); }
